...
use framework\core\SOAServer;

class Server
{
    private $allow_cmd = ['reload', 'restart', 'stop', 'start', 'status', 'forceRestart'];
    private $get_master_cmd = "ps -eaf|grep \"pintuan " . ENV_SYS_NAME . " Server:Master\" | grep -v \"grep\" | awk '{print $2}'";
    private $force_stop_cmd = "ps -eaf|grep \"pintuan " . ENV_SYS_NAME . " Server\" | grep -v \"grep\" | awk '{print $2}' | xargs kill -9";
    private $config = null;
    private $cmd = null;
    ...
    public function __construct($config = [], $argc = null, $argv = [])
    {
        ...
        if ($argc != 2 || (isset($argv[1]) && !in_array($argv[1], $this->allow_cmd))) {
            ...
            echo "    forceRestart:强制重启服务" . PHP_EOL;
    ...
    private function start()
    {
        ...
        if ($master_pid) {
            ...
            echo "服务启动成功" . PHP_EOL;
            $application = new SOAServer($this->config);
            $application->serve();
        }
    ...
    {
        echo "服务正在停止..." . PHP_EOL;
        shell_exec($this->force_stop_cmd);
        echo "服务已经停止..." . PHP_EOL;
        sleep(2);
        $this->start();
    }
