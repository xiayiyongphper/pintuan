...
namespace service\components\bg2gb;
 ...
 class big2gb
{
     ...
     function chg_utfcode($str,$charset='big5')
	{
		if ($charset=='big5')
		{
			$fd = fopen($this->_dir."/gb2big.map",'r');
			$str1 = fread($fd,filesize($this->_dir."/gb2big.map"));
		}
		else 
		{
			$fd = fopen($this->_dir."/big2gb.map",'r');
			$str1 = fread($fd,filesize($this->_dir."/big2gb.map"));
		}
		fclose($fd);
