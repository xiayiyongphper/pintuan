...
/**
 * Class CacheObject
 * @package framework\components\cache
 */
class CacheObject implements \ArrayAccess
{

    /**
     * CacheObject attributes
     *
     * @var array
     */
        ...
        if (strpos($key, '/')) {
            ...
            foreach ($keyArr as $i => $k) {
                ...
                if (is_array($data)) {
                    ...
                    $data = $data[$k];
                } elseif ($data instanceof CacheObject) {
                    $data = $data->getData($k);
                } else {
                    return $default;
                }
        ...
        if (isset($this->_data[$key])) {
            ...
            if (is_array($value)) {
                ...
                return (isset($arr[$index]) && (!empty($arr[$index]) || strlen($arr[$index]) > 0))
                    ? $arr[$index] : null;
            } elseif ($value instanceof CacheObject) {
