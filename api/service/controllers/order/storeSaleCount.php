...
use service\callService\order\StoreSaleCountProxy;

/**
 * Class createOrder
 */
class storeSaleCount extends ApiAbstract
{
    public function run($params)
    {
        if (!empty($params)) {
            $this->doInit($params, true, 2);
            $result = (new StoreSaleCountProxy('order', 'store.storeSaleCount', $this->_request))->sendRequest();
