...
use service\callService\order\ArrivalOrderListProxy;

/**
 * Class createOrder
 */
class arrivalOrderList extends ApiAbstract
{
    public function run($params)
    {
        if (!empty($params)) {
            $this->doInit($params, true, 2);
            $result = (new ArrivalOrderListProxy('order', 'store.arrivalOrderList', $this->_request))->sendRequest();
    ...
    protected function getRules()
    {
        return [
            'main' => [
                ...
                ['status', validParam::CHECK_TYPE_OPTIONAL, validParam::VALUE_TYPE_INT],
