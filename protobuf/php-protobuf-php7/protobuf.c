...
static int pb_set_from(zval *this, zval *params);
static int pb_set_value(zval *this, zval *dst, zval *src, zend_ulong field_number);
static zend_bool pb_get_field_repeated(zval *this, zval *field_descriptor, zend_ulong field_number);
...
PHP_METHOD(ProtobufMessage, setFrom)
{
	zval *params;
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "a", &params) == FAILURE) {
		RETURN_THIS();
	}
	pb_set_from(getThis(),params);
	RETURN_THIS();
}
...
ZEND_BEGIN_ARG_INFO_EX(arginfo_setFrom, 0, 0, 1)
...
static zend_function_entry pb_methods[] = {
	...
	PHP_ME(ProtobufMessage, setFrom, arginfo_setFrom, ZEND_ACC_PUBLIC)
...
{
	zval *field_repeated;
	zend_bool repeated = 0;
    TSRMLS_FETCH();

    field_repeated = zend_hash_str_find(Z_ARRVAL_P(field_descriptor), PB_FIELD_REPEATED, sizeof(PB_FIELD_REPEATED) - 1);
    if (field_repeated!=NULL && Z_TYPE_P(field_repeated) == IS_TRUE){
//    	php_var_dump(field_repeated,1);
    	repeated = 1;
    }
	return repeated;
}
...
static int pb_set_from(zval *this,zval *params)
{
	...
	zend_string *key;
    zval *prop;
    zval *old_value, *values;
    zend_string *const_name;
	if(Z_TYPE_P(params) == IS_REFERENCE){
		params = Z_REFVAL_P(params);
	}

	if(Z_TYPE_P(params) != IS_ARRAY){
		PB_COMPILE_ERROR_EX(this, "setFrom expects parameter 1 to be array",1);
		goto fail0;
	}

	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(params), num_key, key, value) {
		   if (key) { //HASH_KEY_IS_STRING
			   const_name = strpprintf(0, "self::%s", ZSTR_VAL(key));
			   prop = zend_get_constant_ex(const_name, Z_OBJCE_P(this), ZEND_FETCH_CLASS_SILENT);
			   zend_string_release(const_name);
			   if(prop == NULL){
//			       zend_error(E_NOTICE, "class %s does not have constant '%s'", ZSTR_VAL(Z_OBJCE_P(this)->name), ZSTR_VAL(key));
//				   zend_throw_exception_ex(NULL, 0 TSRMLS_CC, "class %s does not have constant '%s'", ZSTR_VAL(Z_OBJCE_P(this)->name), ZSTR_VAL(key));
//				   goto fail0;
                   continue;
			   }
			   if (Z_TYPE_P(prop) == IS_LONG) {
				   field_number = Z_LVAL_P(prop);
			   }
			   zval_ptr_dtor(prop);
			   if(field_number==-1){
//			       zend_error(E_NOTICE, "class %s does not have constant '%s'", ZSTR_VAL(Z_OBJCE_P(this)->name), ZSTR_VAL(key));
//				   zend_throw_exception_ex(NULL, 0 TSRMLS_CC, "class %s does not have constant '%s'", ZSTR_VAL(Z_OBJCE_P(this)->name), ZSTR_VAL(key));
			   ...
			   if ((values = pb_get_values(this)) == NULL)
			   	   goto fail0;

			   	if ((old_value = pb_get_value(this, values, (zend_ulong)field_number)) == NULL)
			   	   goto fail1;
			   	...
			   	if (Z_TYPE_P(value) == IS_NULL) {
			   		...
			   		zval_ptr_dtor(old_value);
			   		if(pb_set_value(this, old_value, value, (zend_ulong)field_number)==FAILURE){
			   			goto fail0;
			   		}
			   	}
		   }
...
//	php_var_dump(this,1);
	...
	repeated = pb_get_field_repeated(this, field_descriptor, field_number);
//	php_var_dump(src,1);
	ZVAL_DUP(&tmp, src);
	if (Z_TYPE_P(type) == IS_LONG) {
		if(repeated==TRUE){
			zval *value,value_tmp;
			zval repeated_value;
			ulong num_key;
			zend_string *key;
			if(Z_TYPE(tmp) != IS_ARRAY){
				PB_COMPILE_ERROR_EX(this, "field '%s' expects value to be array ", pb_get_field_name(this, field_number));
				goto fail0;
			}
			array_init(&repeated_value);
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL(tmp),num_key,key, value) {
				if(key!=NULL){
					PB_COMPILE_ERROR_EX(this, "unexpected '%s' field name '%s' in field descriptor", pb_get_field_name(this, field_number), ZSTR_VAL(key));
					goto fail0;
				}
				ZVAL_DUP(&value_tmp, value);
				switch (Z_LVAL_P(type))
				{
					case PB_TYPE_DOUBLE:
					case PB_TYPE_FLOAT:
						if (Z_TYPE(value_tmp) != IS_DOUBLE)
							convert_to_explicit_type(&value_tmp, IS_DOUBLE);
						break;

					case PB_TYPE_FIXED32:
						if (Z_TYPE(value_tmp) != IS_LONG)
							convert_to_explicit_type(&value_tmp, IS_LONG);
						break;

					case PB_TYPE_BOOL:
						if (!IS_BOOL(value_tmp)) {
							convert_to_boolean(&value_tmp);
						}
						break;
					...
					case PB_TYPE_FIXED64:
					case PB_TYPE_SIGNED_INT:
						if ((Z_TYPE(value_tmp) == IS_DOUBLE) && IS_32_BIT) {
							if ((double)ZEND_LONG_MAX < Z_DVAL(value_tmp)) {
								// store big value for 32bit systems as double
								break;
							}
						}
					...
					case PB_TYPE_STRING:
						if (Z_TYPE(value_tmp) != IS_STRING)
							convert_to_explicit_type(&value_tmp, IS_STRING);
				...
				add_next_index_zval(&repeated_value,&value_tmp);
			}ZEND_HASH_FOREACH_END();
			zval_ptr_dtor(&tmp);
			*dst = repeated_value;
		}else{
		...
		if(repeated==TRUE){
			zend_string *class_name = zval_get_string(type);
			zval *value,zv;
			zval repeated_obj;
			array_init(&repeated_obj);
			ulong num_key;
			zend_string *key;
			zval obj;
			...
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL(tmp),num_key,key, value) {
				...
				zend_call_method_with_0_params(&obj, class_entry, NULL, "reset", &zv);
//				php_var_dump(&obj,1);
//				php_var_dump(value,1);
				if(pb_set_from(&obj,value)==FAILURE){
					goto fail0;
				}
				add_next_index_zval(&repeated_obj,&obj);
			} ZEND_HASH_FOREACH_END();
			zval_ptr_dtor(&tmp);
			*dst = repeated_obj;
		}else{
...
//			php_var_dump(&obj,1);
//			php_var_dump(&tmp,1);
			if(pb_set_from(&obj,&tmp)==FAILURE){
				goto fail0;
			}
			zval_ptr_dtor(&tmp);
			*dst = obj;
...
//	php_var_dump(this,1);
	zval_ptr_dtor(&field_descriptors);
//	zval_ptr_dtor(field_descriptor);
//	zval_ptr_dtor(&tmp);
//	zval_ptr_dtor(type);
	return SUCCESS;
fail2:
